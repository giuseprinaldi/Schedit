...
const POSITION_VALUES = POSITIONS;
...
const shiftSchema = z.object({
  ...
  position: z.enum(POSITION_VALUES as any),
...
export function ShiftModal({ open, onClose, onSave, editingShift, preselectedDate }: ShiftModalProps) {
  ...
  }, [editingShift, preselectedDate, reset]);
  ...
  const onSubmit = async (data: ShiftFormData) => {
    try {
      ...
      if (!res.ok) {
        ...
        toast({ title: "Error", description: err.error ?? "Failed to save shift.", variant: "destructive" });
        return;
      }
            ...
            >
              ...
              {employees.map((emp) => (
                <option key={emp.id} value={emp.id}>
                  {emp.name} — {positionLabel(emp.position as any)}
                </option>
              ))}
            </select>
